function varargout = get_refpoints_on_the_fly(varargin)
% GET_REFPOINTS_ON_THE_FLY M-file for get_refpoints_on_the_fly.fig
%      GET_REFPOINTS_ON_THE_FLY, by itself, creates a new GET_REFPOINTS_ON_THE_FLY or raises the existing
%      singleton*.
%
%      H = GET_REFPOINTS_ON_THE_FLY returns the handle to a new GET_REFPOINTS_ON_THE_FLY or the handle to
%      the existing singleton*.
%
%      GET_REFPOINTS_ON_THE_FLY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GET_REFPOINTS_ON_THE_FLY.M with the given input arguments.
%
%      GET_REFPOINTS_ON_THE_FLY('Property','Value',...) creates a new GET_REFPOINTS_ON_THE_FLY or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before get_refpoints_on_the_fly_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to get_refpoints_on_the_fly_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help get_refpoints_on_the_fly
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @get_refpoints_on_the_fly_OpeningFcn, ...
                   'gui_OutputFcn',  @get_refpoints_on_the_fly_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% Figure Control
%====================================================
function get_refpoints_on_the_fly_OpeningFcn(hObject, eventdata, handles, varargin)
% 
handles.output = hObject;
setappdata(handles.figure1,'SaveFlag',0);
  ...
  for index=1:2:(nargin-3),
    ...
    switch lower(varargin{index})
      case 'inifile'
        ...
        fclose(fid);
%    case 'parent_serial'
%        setappdata(handles.figure1,'Parent_Serial',varargin{index+1});
    end
  end
else
  msgbox('IniFile is not specified.');
  msgbox('Close Get Ref Pos Window.');
  return;
end
...
% UIWAIT makes get_refpoints_on_the_fly wait for user response (see UIRESUME)
...
%    s = getappdata(handles.figure1,'Parent_Serial');
    fopen(s);
    %readasync(s);
...
% ISO TRAKII mode is set by upper module. hold the mode.
%    fprintf(s,'Y');
%    fprintf(s,'u');
%    fprintf(s,'E');
%    CtrlD=char(4);
...
function varargout = get_refpoints_on_the_fly_OutputFcn(hObject, eventdata, handles)
%
% Get default command line output from handles structure
%varargout{1} = handles.output;
varargout{1}=getappdata(handles.figure1,'SaveFlag');
delete(get(0,'CurrentFigure'));
...
ret=Ref_file_save_current(dpath, fname, Name, Id, Sex, Age, Comment, pos_Cz, pos_Nz, pos_Iz, pos_AR, pos_AL);
if(ret==1)
    setappdata(handles.figure1,'SaveFlag',1);

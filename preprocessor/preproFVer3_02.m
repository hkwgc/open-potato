function varargout = preproFVer3_02(fcn, varargin)
% Preprocessor Function of FVer 3.02/3.03 Fromat: Normal..
%   This Format Read FVer3_02.
%   And Plug-in Format Version 1.0 
%
% ** CREATE BASIC INFORMATION **
% Syntax:
%  basic_info = preproFVer3_02('createBasicInfo');
...
% Syntax:
%  [flag,msg] = preproFVer3_02('CheckFile',filename)
...
% Syntax:
%  str = preproFVer3_02('getFileInfo',filename);
...
% Syntax:
%  str = preproFVer3_02('getFunctionInfo')
%     str          : Information of the function.
%                    Cell array of string.
%
% ** execute **
% Syntax:
%  [hdata, data] = preproFVer3_02('Execute', filename)
%  Continuous Data. 
%
%  OT_LOCAL_DATA = preproFVer3_02('ExecuteOld', filename)
...
% create : 2005.12.21
% $Id: preproFVer3_02.m 180 2011-05-19 09:34:28Z Katura $
...
% ** CREATE BASIC INFORMATION **
% Syntax:
%  basic_info = preproFVer3_02('createBasicIno');
%    Return Information for OSP Application.
%    basic_info is structure that fields are
%       DispName : Display Name : String
%       Version  : Version of the function
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % disp(line_01);
  % disp(line_01{1});
  flag = strncmp(line_01{1},'FVer 3.02',9);
  if flag==false,
    flag = strncmp(line_01{1},'FVer 3.03',9);
    if flag==false,
      msg='1st Line is not Header of FVer 3.02/3.03';
    end
  end
return;    
...
%     filename     : Full-Path of the File.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  str={[' Format FVer 3.02/3.03 : ' filename]};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 [pathname, fname, ext] = fileparts(filename);
  ...
  [dum]=textread(filename,'%d%*[^\n]',9,'headerlines',5,'whitespace', ...
		 '''');
  sex=dum(1);opmode=dum(2);anamode=dum(3);measmode=dum(4);
  dspslct=dum(5);hbslct=dum(6);wavslct=dum(7);timescl=dum(8);rate= ...
	  dum(9);
  clear dum;

  [stimselect(1), stimselect(2)] = ...
      textread(filename,'%d%d%*[^\n]',1,'headerlines',14,'whitespace', ...
	       '''');
 ...
 %%+++ Read data-block
  ...
  [adnmpn]=(textread(filename,'%d'  ,adn,'headerlines',131+shift, ...
		     'delimiter',','))';
  [wlen]  =(textread(filename,'%20f',adn,'headerlines',132+shift, ...
		     'delimiter',','))';
  [adrng] =(textread(filename,'%20f',adn,'headerlines',133+shift, ...
		     'delimiter',','))';
  [amprng]=(textread(filename,'%20f',adn,'headerlines',134+shift, ...
		     'delimiter',','))';

  [dum]=textread(filename,'%20f ','headerlines',140+shift,'delimiter',', ');
 ...
 %Change order of colom A/D to Meas point
  ...
  datanum=length(data(:,1));tstp=smpl_period/1000;time=[0:tstp: ...
		    (datanum-1)*tstp];

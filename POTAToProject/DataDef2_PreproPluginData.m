function varargout=DataDef2_PreproPluginData(mode, key)
% Data Definition of Block Filter Data Result
%
%  Keys = DataDef2_PreproPluginData('getKeys')
%     return Cell structure of Search Key of FilterData Data
%  [Keys, tag] = DataDef2_PreproPluginData('getKeys')
%     tag is Example of Search keys
%     correspond to Keys
%
%
%  [id_fieldname] = DataDef2_PreproPluginData('getIdentifierKey')
%     Identifier of the Data.
%     This Key must be unique
%
%  [filelists]  = DataDef2_PreproPluginData('loadlist')
%     Load File List
%
%  [info] = DataDef2_PreproPluginData('showinfo',key)
%     info is 'Cell Array of  string'.
%             strings is a file-list information.
%
%  [FilterData ] = DataDef2_PreproPluginData('load')
%  [FilterData ] = DataDef2_PreproPluginData('load',activedata)
%     Load File List from Active Data.
%       -> and set corresponding data.
%
%  DataDef2_PreproPluginData('save')
%     Save Current Data.
%
%  DataDef2_PreproPluginData('save_ow')
%     Save Current Data -> Over-Write Mode.
%
%  [filename] = DataDef2_PreproPluginData('make_mfile',key)
%      key :
%        ( case 1)
...
%
%  [data, header] = DataDef2_PreproPluginData('make_ucdata',key)
...
%
%  [strHBdata, plot_kind] =
%          DataDef2_PreproPluginData('make_timedata', key)
%      Make HBdata for plot, and using plot
%
% ---> Special Key
%  groupdata = DataDef2_PreproPluginData('NewFilter', key)
%      Make New Group-Data
%      key : Save FilteData
%
%  [info linfo kindlist] = DataDef2_PreproPluginData('showblocks',key)
...
% create : 2007.01.31
% $Id: DataDef2_PreproPluginData.m 180 2011-05-19 09:34:28Z Katura $
%
% Reversion 1.10
%    * Change Print Unit of time.
%    * (Easter egg)
%      BasicData for DataDef2_PreproPluginData.
...
switch mode
  ...
  case 'NewFilter'
    ...
    while(ng_flag),
      ...
      try
        DataDef2_PreproPluginData('save', gdata);
        ng_flag=0;
      catch
  ...
  case 'save'
    ...
    gdatalist = DataDef2_PreproPluginData('loadlist');
    if ~isempty(gdatalist)
      c = struct2cell(gdatalist);
      f = fieldnames(gdatalist);
      p = strcmp(f,Keys{IdentifierID});
      c = squeeze(c(p,:,:));
      % if exist Cancel
  ...
  case 'save_ow'
    ...
    if ~isempty(gdatalist)
      ...
      id = strcmp(c,getfield(gdata,Keys{IdentifierID}))==1;
      clear c f p;
    else
...
if ~isrun,
  error(['[E] ' mfilename ' : Plutform is no running']);
end
...
fname =[path0 ...
  filesep pj.Name ...
  filesep 'DataListPreproPluginData.mat'];
return;

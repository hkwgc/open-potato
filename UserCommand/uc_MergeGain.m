function [data, header] = uc_MergeGain(data,header),
...
% create : 2005.07.21
% $Id: uc_MergeGain.m 180 2011-05-19 09:34:28Z Katura $
%
...
%   No check..
  ...
  data(:,:,end+1)=repmat(tag.ampgain(:,1:2:size_of_raw(2)-1),[size(data,1) 1 1]);
  header.TAGs.DataTag{end+1} = sprintf('Raw %d nm',ceil(median(tag.wavelength(1:2:size_of_raw(2)-1))));

  data(:,:,end+1)=repmat(tag.ampgain(:,2:2:size_of_raw(2)),[size(data,1) 1 1]);
  header.TAGs.DataTag{end+1} = sprintf('Raw %d nm',ceil(median(tag.wavelength(2:2:size_of_raw(2)))));

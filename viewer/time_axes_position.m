function [psn] = time_axes_position(pos_data, pa_size, pa_pos, testflg)
...
% create :
% $Id: time_axes_position.m 180 2011-05-19 09:34:28Z Katura $
%
% reversion 1.1:
%  Bug fixed by hisa
%
...
switch measure_mode
  ...
  case 201,
    ...
    if 1 % 2010.11.18
      ...
      sz=[width0, height0];
      x=0:14;  x=x*width0;
      y=2:-1:0;   y=y*height0; % 2011.03.10
    else
      width0  = 0.9/15;
      height0 = 0.9/3;
      sz=[width0*0.9, height0*0.8]; % insert space of each axes
  ...
  otherwise
    ...
    if isstruct(pos_data),
      ...
      switch nargin
        case 1,
          [psn] = time_axes_position(pos_data);
        case 2,
          [psn] = time_axes_position(pos_data, pa_size);
        case 3,
          [psn] = time_axes_position(pos_data, pa_size, pa_pos);
        otherwise
          [psn] = time_axes_position(pos_data, pa_size, pa_pos, testflg);
      end
      return;
    else
...
if nargin >= 4, % if exist testflag, plot test
  ...
  chid=1;
  for lpsn = 1:size(psn,1)
    axes('position', psn(lpsn,:));

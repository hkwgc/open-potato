...
% Get Callback Data
data=p3_ViewCommCallback('getData',ud.axes,ud.name,ud.ObjectID);
%--> Object Data of PLOT-TEST <--
for idx=2:2:length(varargin)
  try
    eval(['data.curdata.' varargin{idx-1} '=varargin{idx};']);
  catch
...
try
  delete(data.handle(ishandle(data.handle)));
catch
  warning(C__FILE__LINE__CHAR);
end
...
try
  ...
  warning(C__FILE__LINE__CHAR);
  rethrow(lasterror);
  testimage(data.axes, data.curdata, data.obj ,ud.ObjectID);
end
return;

function [c x y z] = ch23dimageXX(ydata,psn,image_mode,v_interpstep, v_interpmethod)
%modified by K.Nakajo 20100319
%
% shift Z-direction upper by shiftZ
%
[th,phi,r] = cart2sph(psn(:,1),psn(:,2),psn(:,3));
shiftZ = min(r);
xyz1=psn(:,1:3)';
xyz1(3,:)=xyz1(3,:)+shiftZ;
%
% rotate data around y-axis by -90degree
%
rotate_around_y = -0.5*pi; % -90degree
M_rot_y = [cos(rotate_around_y), 0.0, -sin(rotate_around_y);
           0.0, 1.0, 0.0;
           sin(rotate_around_y), 0.0, cos(rotate_around_y)];
xyz2 = (M_rot_y*xyz1)';
% By above tanslation, xyz2 coordinates translated into
% x>0 region. i.e. (-pi<theta<pi,-pi<phi<pi) region

% -- TODO : mode 1, 3
%[th,phi,r] = cart2sph(psn(:,1),psn(:,2),psn(:,3));
[th,phi,r] = cart2sph(xyz2(:,1),xyz2(:,2),xyz2(:,3));
% modify part1 up to here by K.Nakajo 20100309
...
if 0 % no-need this correction by K.Nakajo 20100319
% Grid
...
switch image_mode % switch Image Mode
  ...
  case 2 % Image Mode = INTERPED
    ...
    x3(:)=tmp(1,:);y3(:)=tmp(2,:);
    %<-------------------------
    % modified part2 by K.Nakajo 20100319
    %[x,y,z]=sph2cart(x3,y3,r3);
    [x2,y2,z2]=sph2cart(x3,y3,r3);
    % -rotate_around_y:+90degree
    M_rot_y = [cos(-rotate_around_y), 0.0, -sin(-rotate_around_y);
           0.0, 1.0, 0.0;
           sin(-rotate_around_y), 0.0, cos(-rotate_around_y)];

    [row_size col_size] = size(x2);
    % rotate inverse y-axis & shift down z-direction
    for i=1:1:row_size
        temp = M_rot_y*[x2(i,:);y2(i,:);z2(i,:)];
        temp(3,:)=temp(3,:)-shiftZ;
        x(i,:) = temp(1,:); y(i,:) = temp(2,:); z(i,:) = temp(3,:);
    end
    % modified part2 up to here by K.Nakajo 20100319
    c      =v;

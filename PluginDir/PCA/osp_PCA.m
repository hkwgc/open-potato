...
function osp_PCA_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = osp_PCA_OutputFcn(hObject, eventdata, handles)
...
function psb_ok_Callback(hObject, eventdata, handles)
  cmp=str2num(get(handles.edtComps,'String'));
...
function varargout=psb_LoadData_Callback(hObject, eventdata, handles)
...
function psb_plot_Callback(hObject, eventdata, handles)
  ...
  if get(handles.ckb_plot_rstructdata,'Value')
    ...
    ch=str2num(get(handles.edtShowCh,'String'));
    spz=size(ch,2);
    figure;
    for ich=1:spz
  ...
  if get(handles.ckb_score,'Value')
    ...
    cp1 = str2num(cp1_str);
    cp2 = str2num(cp2_str);
    if length(cp1)~=1 length(cp2)~=1
      errordlg(' Compornent Setting Error');
    else      
      figure;

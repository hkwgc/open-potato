function varargout = uiFileSelect(varargin)
% UIFILESELECT M-file for uiFileSelect.fig
%      UIFILESELECT, by itself, creates a new UIFILESELECT or raises the existing
%      singleton*.
%
%      H = UIFILESELECT returns the handle to a new UIFILESELECT or the handle to
%      the existing singleton*.
%
%      UIFILESELECT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in UIFILESELECT.M with the given input arguments.
%
%      UIFILESELECT('Property','Value',...) creates a new UIFILESELECT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before uiFileSelect_OpeningFunction gets called.  An
...
% Edit the above text to modify the response to help uiFileSelect

% Last Modified by GUIDE v2.5 09-May-2006 12:05:14

% == History ==
% Original author : Masanori Shoji
% create : 2005.01.17
% $Id: uiFileSelect.m 180 2011-05-19 09:34:28Z Katura $

% Reversion 1.9
%   if no data in 'FileType',
%   select other FilteType in default.
...
if OSP_DATA==0
  warndlg('Please Open OSP at First!','Cannot OPEN');
  return;
end
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @uiFileSelect_OpeningFcn, ...
                   'gui_OutputFcn',  @uiFileSelect_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%            Opening Function                 %
% ******************************************* %
function uiFileSelect_OpeningFcn(hObject, eventdata, handles, varargin)
% Open uiFileSelect 
...
% Like Open function..
  try   
    ...
    for argid=2:2:length(varargin)
      ...
      switch prop
       case 'SetMax',
	set(handles.lb_FileList, 'Max', val);
	
       case {'DataDif','DataDef'},
	set(handles.pop_FileType,'String',val);
       otherwise,
    ...
    errordlg([ 'In uiFileSelect Opening Function :'...
	       lasterr]);
  end
  ...
  try
    ...
    if isempty(actflag), actflag=0; end
    OSP_DATA('SET','ACTIVE_FLAG',bitset(actflag,1,1));
  end
...
%             Create Function                 %
% ******************************************* %
function lb_FileInfo_CreateFcn(hObject, eventdata, handles)
...
%   --> Restart uiFileSelect
%   Here set Application Data "DataDefFcn"
%                             "DataList"
  ftype0 = get(hObject,'String');
  fid    = get(hObject,'Value');
  ftype  = ftype0{fid};

  % local-data:  Befor Selection id check
  fid_old=getappdata(hObject, 'OLD_FTYPE');
  if fid==fid_old,  return;  end

  setappdata(hObject, 'OLD_FTYPE',fid);
  
  %% Select Data Define Function
  fcn=['DataDef_' ftype];
  fcn2=['DataDef2_' ftype];
  if exist(fcn,'file')
    fcn=eval(['@' fcn]);
  elseif exist(fcn2,'file')
    fcn=eval(['@' fcn2]);
  else
...
%             Close Function                  %
% ******************************************* %
function varargout = uiFileSelect_OutputFcn(hObject, eventdata, handles)
...
function figure1_DeleteFcn(hObject, eventdata, handles)
  try
    % File Lock
    actflag = OSP_DATA('GET','ACTIVE_FLAG');
    OSP_DATA('SET','ACTIVE_FLAG',bitset(actflag,1,0));
  end

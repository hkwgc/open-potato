function varargout = UI_INNERKEY(varargin)
% UI_INNERKEY M-file for UI_INNERKEY.fig
%      UI_INNERKEY, by itself, creates a new UI_INNERKEY or raises the existing
%      singleton*.
%
%      H = UI_INNERKEY returns the handle to a new UI_INNERKEY or the handle to
%      the existing singleton*.
%
%      UI_INNERKEY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in UI_INNERKEY.M with the given input arguments.
%
%      UI_INNERKEY('Property','Value',...) creates a new UI_INNERKEY or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before UI_INNERKEY_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to UI_INNERKEY_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help UI_INNERKEY
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @UI_INNERKEY_OpeningFcn, ...
                   'gui_OutputFcn',  @UI_INNERKEY_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before UI_INNERKEY is made visible.
function UI_INNERKEY_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for UI_INNERKEY
...
% UIWAIT makes UI_INNERKEY wait for user response (see UIRESUME)
 uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = UI_INNERKEY_OutputFcn(hObject, eventdata, handles) 
try

function varargout = group_layout_lineprop(varargin)
% GROUP_LAYOUT_LINEPROP M-file for group_layout_lineprop.fig
%      GROUP_LAYOUT_LINEPROP, by itself, creates a new GROUP_LAYOUT_LINEPROP or raises the existing
%      singleton*.
%
%      H = GROUP_LAYOUT_LINEPROP returns the handle to a new GROUP_LAYOUT_LINEPROP or the handle to
%      the existing singleton*.
%
%      GROUP_LAYOUT_LINEPROP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GROUP_LAYOUT_LINEPROP.M with the given input arguments.
%
%      GROUP_LAYOUT_LINEPROP('Property','Value',...) creates a new GROUP_LAYOUT_LINEPROP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before group_layout_lineprop_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to group_layout_lineprop_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help group_layout_lineprop
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @group_layout_lineprop_OpeningFcn, ...
                   'gui_OutputFcn',  @group_layout_lineprop_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% GUI I/O Functions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function group_layout_lineprop_OpeningFcn(hObject, eventdata, handles, varargin)
  handles.output = hObject;
  handles.group_layout_lineprop=hObject;
  handles.figure1=handles.group_layout_lineprop;
  ...
  uiwait(handles.group_layout_lineprop);
return;

function varargout = group_layout_lineprop_OutputFcn(hObject, eventdata, handles)
  % Update handles structure
  handles=guidata(hObject);
  varargout{1} = handles.output;
  delete(handles.group_layout_lineprop);
return;
...
function psb_add_Callback(hObject, eventdata, handles)
  ...
  style = get(handles.pop_lineStyle,'String');
  mark  = mark{get(handles.pop_lineMark, 'Value')};
  str{end+1}=markPlus(addname, mark);
  val=length(str);
  ud{val}.name  = addname;
  ud{val}.mark  = mark;
...
function edit_add_Callback(hObject, eventdata, handles)
  ...
  if ~isempty(val) && isempty(find(1<=val&val<=3)),
    for i=1:length(val),
      str{val(i)}=markPlus(addname, mark);
...
function psb_set_Callback(hObject, eventdata, handles)
  ...
  %  Resume 
  if isequal(get(handles.group_layout_lineprop, 'waitstatus'), 'waiting')
    uiresume(handles.group_layout_lineprop);
  else
    delete(handles.group_layout_lineprop);
  end

  %group_layout_lineprop_deleteFcn(handles.group_layout_lineprop, [],handles);
return;

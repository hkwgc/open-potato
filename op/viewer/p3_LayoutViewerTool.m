function varargout=p3_LayoutViewerTool(fnc,varargin)
% POTATo Layout Viewer Tool
%
% --- Data I/O Function ---
% size=p3_LayoutViewerTool('getCurrentSize',curdata);
%   Get Current Data Size.
%     size=[block-size,time-size, channel-size, data-kind-size];
%     if current is continuous, block-size=0;
%
% [hdata,data]=p3_LayoutViewerTool('getCurrentData',figh,curdata);
%   get Current User-Data : when block Do "Block-Mean" for data
%
% [hdata,data]=p3_LayoutViewerTool('getCurrentDataRaw',figh,curdata);
%   get Current User-Data : (Not Modify)
%
% --- Tag setting ---
%  p3_LayoutViewerTool('setPathTag',handle,curdata);
%    set Tag (from path) to handle.
%
% --- Add menu ---
%  p3_LayoutViewerTool('addMenu_Edit_Axes0',curdata);
%     Add Edit-Axes-Menu to CurrentData menu.
%
%  p3_LayoutViewerTool('addMenu_Edit_Line',curdata);
...
% -- Inner sub function --
% ------------------------
%  strt=p3_LayoutViewerTool('make_pathstr',pathidx_vector);
%      Make String of Path-Index
%      This function is to make TAG
%      Example:
%         str=p3_LayoutViewerTool('make_pathstr',[1:3]);
% ------------------------
...
% create : 2006.04.05
% $Id: p3_LayoutViewerTool.m 352 2013-05-08 02:42:56Z Katura $
%
% Revision 1.4
...
switch mode
  case 1
    mh=uimenu(nowmenu,'Label','Apply GCA X-axis to axes',...
      'Callback',...
      ['a0=p3_LayoutViewerTool(''findAxes'',gcbf,''' ...
      char(tagkey) ''');'...
      'p3_LayoutViewerTool(''addMenu_EditAxisProperty'',1,a0);']);
    mh=uimenu(nowmenu,'Label','Apply GCA Y-axis to axes',...
      'Callback',...
      ['a0=p3_LayoutViewerTool(''findAxes'',gcbf,''' ...
      char(tagkey) ''');'...
      'p3_LayoutViewerTool(''addMenu_EditAxisProperty'',2,a0);']);
    mh=uimenu(nowmenu,'Label','Apply GCA XY-axis to axes',...
      'Callback',...
      ['a0=p3_LayoutViewerTool(''findAxes'',gcbf,''' ...
      char(tagkey) ''');'...
      'p3_LayoutViewerTool(''addMenu_EditAxisProperty'',3,a0);']);
    mh=uimenu(nowmenu,'Label','Tight Axis',...
      'Separator','on',...
      'Callback',...
      ['a0=p3_LayoutViewerTool(''findAxes'',gcbf,''' ...
      char(tagkey) ''');'...
      'p3_LayoutViewerTool(''addMenu_EditAxisProperty'',4,a0);']);
    mh=uimenu(nowmenu,'Label','Reset Axis',...
      'Callback',...
      ['a0=p3_LayoutViewerTool(''findAxes'',gcbf,''' ...
      char(tagkey) ''');'...
      'p3_LayoutViewerTool(''addMenu_EditAxisProperty'',5,a0);']);
  otherwise,
...
set(mh,'Callback',...
  ['a0=p3_LayoutViewerTool(''findAxes'',gcbf,''' ...
  char(tagkey) ''');' ...
  'p3_LayoutViewerTool(''addMenu_AxesTitleSetting'',1,a0);'...
  ]);
...
set(mh,'Callback',...
  ['a0=p3_LayoutViewerTool(''findAxes'',gcbf,''' ...
  char(tagkey) ''');' ...
  'p3_LayoutViewerTool(''addMenu_AxesTitleSetting'',2,a0);'...
  ]);
return;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
uimenu(h,'Label','Line &Color', ...
  'Callback',...
  ['p3_LayoutViewerTool(''lineprop'', gcbf, ''' ...
...
uimenu(h,'Label','&Marker', ...
  'Callback',...
  ['p3_LayoutViewerTool(''MarkerProp'', gcbf, ''' ...
...
uimenu(h,'Label','on', ...
  'Callback',...
  ['p3_LayoutViewerTool(''Legend'', gcbf, ''' ...
  char(tagkey) ''', ''on'');']);
uimenu(h,'Label','off', ...
  'Callback',...
  ['p3_LayoutViewerTool(''Legend'', gcbf, ''' ...
